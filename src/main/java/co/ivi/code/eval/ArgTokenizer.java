...
public class ArgTokenizer {
    ...
    public ArgTokenizer(String prefix, String arg) {
        this.str = arg;
        this.prefix = prefix + " ";
    ...
    private void foundOption(String opt) {
        ...
        List<Map.Entry<String,Boolean>> matches =
                options.entrySet()
                       ...
                       .toList();
        if (matches.size() == 1) {
            matches.getFirst().setValue(true);
        } else {
            badOptions.add(opt);
        }
    }
     ...
     */
    public void allowedOptions(String... opts) {
        for (String opt : opts) {
     ...
     */
    public boolean isAllowedOption(String opt) {
